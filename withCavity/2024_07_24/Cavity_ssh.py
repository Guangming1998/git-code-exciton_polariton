...
class Trajectory_SSHmodel():
    ...
    def initialHamiltonianCavity(self,couplingStrength,cavityFrequency):
        ...
        for i in range(self.Nmol):
            ...
            matrix_Jcav[i,0] = -1j*self.couplingStrength 
    ...
    def initialGaussian_ensemble(self,index):

        Xj = self.Xj
        Vj = self.Vj
        
        self.Xj = np.zeros((self.Nmol,self.Nmol))
        self.Vj = np.zeros((self.Nmol,self.Nmol))
    ...
    def initialCj_cavity_trajectory(self,kBT):
        Hmol = self.Htot
        W, U = np.linalg.eigh(Hmol)
        ...
        self.Prob = Prob/np.sum(Prob)
        # print(self.Prob)
        # print(np.sum(self.Prob[:35]))
        
        init = 1 - 1
        self.Cj = U[:,init]
        self.Prob = self.Prob[init]
    
    def initialCj_cavity_ensemble(self,kBT):
        Hmol = self.Htot
        W, U = np.linalg.eigh(Hmol)
        ...
        self.Prob = Prob/np.sum(Prob)
        index = 35
        self.Prob = self.Prob[:35]/np.sum(self.Prob[:35])
        
        self.Cj = U
        
    def initialCj_Troisi_ensemble(self,kBT):
        Hmol = self.Hmol
        W, U = np.linalg.eigh(Hmol)
    ...
    def old_Aj_ensemble(self,index,Ehrenfest=True):  #calculate a(t). update on 2023_4_18
        Aj = -self.Kconst/self.mass * self.Xj
        # print('This is Aj without Ehrenfest')
        # print(Aj)
        ...
        if Ehrenfest:
            for it in range(index):
                for j in range(1,self.Nmol-1):
                    Aj[j,it] = Aj[j,it] -self.dynamicCoup/(self.mass)* \
                            ( 2*np.real(np.conj(self.Cj[j,it])*self.Cj[j-1,it]) \
                            - 2*np.real(np.conj(self.Cj[j,it])*self.Cj[j+1,it]))
                Aj[0,it] = Aj[0,it] -self.dynamicCoup/(self.mass)* \
                        ( 2*np.real(np.conj(self.Cj[0,it])*self.Cj[-1,it]) \
                        - 2*np.real(np.conj(self.Cj[0,it])*self.Cj[1,it]))
                Aj[-1,it] = Aj[-1,it] -self.dynamicCoup/(self.mass)* \
                            ( 2*np.real(np.conj(self.Cj[-1,it])*self.Cj[-2,it]) \
                            - 2*np.real(np.conj(self.Cj[-1,it])*self.Cj[0,it]))
        # print('This is Aj with Ehrenfest')
        # print(Aj)
        self.accelerate = Aj
    ...
    def velocityVerlet_ensemble(self,dt,index,Ehrenfest=True):
        """
        We use the algorithm with eliminating the half-step velocity
        https://en.wikipedia    .org/wiki/Verlet_integration
        """
    ...
    def RK4_Cj_cavity_trajectory(self,dt):
        
        k1 = -1j*np.dot(self.Htot,self.Cj)
        k2 = -1j*np.dot(self.Htot,self.Cj + dt*k1/2)
        k3 = -1j*np.dot(self.Htot,self.Cj + dt*k2/2)
        k4 = -1j*np.dot(self.Htot,self.Cj + dt*k3)
        
        self.Cj += (k1 + 2*k2 + 2*k3 + k4)*dt/6

    def RK4_Cj_cavity_ensemble(self,dt,index):
        
        for it in range(index):
            k1 = -1j*np.dot(self.Htot,self.Cj[:,it])
            k2 = -1j*np.dot(self.Htot,self.Cj[:,it] + dt*k1/2)
            k3 = -1j*np.dot(self.Htot,self.Cj[:,it] + dt*k2/2)
            k4 = -1j*np.dot(self.Htot,self.Cj[:,it] + dt*k3)
            
            self.Cj[:,it] += (k1 + 2*k2 + 2*k3 + k4)*dt/6
    
    def RK4_Cj_Troisi_ensemble(self,dt,index):
        
        for it in range(index):
            self.updateHmol_Troisi_ensemble(self.Xj[:,it])
            k1 = -1j*np.dot(self.Hmol,self.Cj[:,it])
            k2 = -1j*np.dot(self.Hmol,self.Cj[:,it] + dt*k1/2)
            k3 = -1j*np.dot(self.Hmol,self.Cj[:,it] + dt*k2/2)
            k4 = -1j*np.dot(self.Hmol,self.Cj[:,it] + dt*k3)
    ...
    def updateHmol_Troisi_ensemble(self,Xj):
        for j in range(self.Nmol-1):
            self.Hmol[j,j+1] = -self.staticCoup + self.dynamicCoup * (Xj[j+1]-Xj[j])
            self.Hmol[j+1,j] = -self.staticCoup + self.dynamicCoup * (Xj[j+1]-Xj[j])

        self.Hmol[0,-1] = -self.staticCoup + self.dynamicCoup * (Xj[0]-Xj[-1])
        self.Hmol[-1,0] = -self.staticCoup + self.dynamicCoup * (Xj[0]-Xj[-1])

        
    def updateHamiltonianCavity(self):
        self.Htot = np.vstack((np.hstack((self.Hmol, self.Hcoupling)),
                              np.hstack((self.Hcoupling.T, self.Hcav))))
    ...
    def getCurrentCorrelation_cavity(self):
        self.Ngroundstate = 0
        if hasattr(self, 'Jmol0Cj'):
            # self.Jt = deepcopy(self.Ht)
            self.Jmol_t = np.zeros_like(self.Htot)
            for j in range(self.Nmol-1): 
                self.Jmol_t[self.Ngroundstate+j, self.Ngroundstate+j+1] = -self.Hmol[self.Ngroundstate+j, self.Ngroundstate+j+1] *1j
                self.Jmol_t[self.Ngroundstate+j+1, self.Ngroundstate+j]   = self.Hmol[self.Ngroundstate+j+1, self.Ngroundstate+j] *1j   
            
            self.Jmol_t[self.Ngroundstate, self.Ngroundstate+self.Nmol-1] = self.Hmol[self.Ngroundstate, self.Ngroundstate +self.Nmol-1]*1j
            self.Jmol_t[self.Ngroundstate +self.Nmol-1, self.Ngroundstate] = -self.Hmol[self.Ngroundstate+self.Nmol-1, self.Ngroundstate]*1j 
            self.Jcav_t = deepcopy(self.Jcav_0)
            
            self.Jtot_t = self.Jmol_t + self.Jcav_t
            # Here Cj is at time t
            # self.JtCj = np.dot(self.Jt,self.Cj
        else: #first step only 
            self.Jmol0Cj = np.dot(self.Jmol_0,self.Cj)
            self.Jtot0Cj = np.dot(self.Jtot_0,self.Cj)
            self.Jcav0Cj = np.dot(self.Jcav_0,self.Cj)
            
            self.Jtot_t = deepcopy(self.Jtot_0)
            self.Jmol_t = deepcopy(self.Jmol_0)
            self.Jcav_t = deepcopy(self.Jcav_0)

        CJJmol = np.dot(np.conj(self.Cj).T,np.dot(self.Jmol_t,self.Jmol0Cj))
        CJJcav = np.dot(np.conj(self.Cj).T,np.dot(self.Jcav_t,self.Jcav0Cj))
        CJJtot = np.dot(np.conj(self.Cj).T,np.dot(self.Jtot_t,self.Jtot0Cj))
        
        return CJJmol, CJJcav, CJJtot

    def getCurrentCorrelation_ensemble(self,index):
        self.Ngroundstate = 0
        if hasattr(self, 'Jmol0Cj'):
            # self.Jt = deepcopy(self.Ht)
            self.Jmol_t = np.zeros_like(self.Htot)
        ...
        Cjjmol_each = np.zeros(index,complex)
        Cjjcav_each = np.zeros(index,complex)
        Cjjtot_each = np.zeros(index,complex)
        # print(self.Jmol0Cj[:,0].shape)
        # print(self.Cj[:,0].T.shape)
        # print(self.Jmol_t.shape)
        for it in range(index):
            # print(np.dot(np.conj(self.Cj[:,it]).T, np.dot(self.Jmol_t,self.Jmol0Cj[:,it])) )
            Cjjmol_each[it] = np.dot(np.conj(self.Cj[:,it]).T, np.dot(self.Jmol_t,self.Jmol0Cj[:,it])) * self.Prob[it]
            Cjjcav_each[it] = np.dot(np.conj(self.Cj[:,it]).T, np.dot(self.Jcav_t,self.Jcav0Cj[:,it])) * self.Prob[it]
            Cjjtot_each[it] = np.dot(np.conj(self.Cj[:,it]).T, np.dot(self.Jtot_t,self.Jtot0Cj[:,it])) * self.Prob[it]
        CJJmol = np.sum(Cjjmol_each)
        CJJcav = np.sum(Cjjcav_each)
        CJJtot = np.sum(Cjjtot_each)
        
        return CJJmol, CJJcav, CJJtot
    
    def getCurrentCorrelation_Troisi_ensemble(self,index):
        self.Ngroundstate = 0
        Cjjmol_each = np.zeros(index,complex)
        if hasattr(self, 'Jmol0Cj'):
            # self.Jt = deepcopy(self.Ht)
            for it in range(index):
                self.updateHmol_Troisi_ensemble(self.Xj[:,it])
                self.Jmol_t = np.zeros_like(self.Hmol)
                for j in range(self.Nmol-1): 
                    self.Jmol_t[self.Ngroundstate+j, self.Ngroundstate+j+1] = -self.Hmol[self.Ngroundstate+j, self.Ngroundstate+j+1] *1j
                    self.Jmol_t[self.Ngroundstate+j+1, self.Ngroundstate+j]   = self.Hmol[self.Ngroundstate+j+1, self.Ngroundstate+j] *1j   
            ...
            self.Jmol0Cj = np.dot(self.Jt0,self.Cj)
            self.Jmol_t = deepcopy(self.Jt0)
            for it in range(index):
                Cjjmol_each[it] = np.dot(np.conj(self.Cj[:,it]).T, np.dot(self.Jmol_t,self.Jmol0Cj[:,it])) * self.Prob[it]
    ...
    def getCurrentCorrelation_static(self,dt,Ntimes,kBT,Vcav,J_cav=True,J_mol=True):
    ...
    def getCurrentCorrelation_cavity_trajectory(self,Cj,J0Cj):
        
        self.Jtot_t = deepcopy(self.Jtot_0)
        for j in range(self.Nmol-1): 
            self.Jtot_t[j, j+1] = -self.Hmol[j, j+1]*1j
            self.Jtot_t[j+1, j] = self.Hmol[j+1, j]*1j   
        
        self.Jtot_t[0,self.Nmol-1] = self.Hmol[0, self.Nmol-1]*1j
        self.Jtot_t[self.Nmol-1,0] = -self.Hmol[self.Nmol-1, 0]*1j 
        
        CJJ = np.dot(np.conj(Cj).T,np.dot(self.Jtot_t,J0Cj))
        
        return CJJ
    
    def propagateJ0Cj_RK4_trajectory(self,dt,J0Cj,Vcav):
        ### RK4 propagation 
        if Vcav==0:
            ...
            K1 = -1j*np.dot(self.Htot,J0Cj)
            K2 = -1j*np.dot(self.Htot,J0Cj+dt*K1/2)
            K3 = -1j*np.dot(self.Htot,J0Cj+dt*K2/2)
            K4 = -1j*np.dot(self.Htot,J0Cj+dt*K3)
    ...
    def propagateJmol0Cj_RK4(self,dt):
        ### RK4 propagation 
        K1 = -1j*np.dot(self.Htot,self.Jmol0Cj)
        K2 = -1j*np.dot(self.Htot,self.Jmol0Cj+dt*K1/2)
        K3 = -1j*np.dot(self.Htot,self.Jmol0Cj+dt*K2/2)
        K4 = -1j*np.dot(self.Htot,self.Jmol0Cj+dt*K3)
        self.Jmol0Cj += (K1+2*K2+2*K3+K4)*dt/6

    def propagateJmol0Cj_RK4_ensemble(self,dt,index):
        ### RK4 propagation 
        for it in range(index):
            K1 = -1j*np.dot(self.Htot,self.Jmol0Cj[:,it])
            K2 = -1j*np.dot(self.Htot,self.Jmol0Cj[:,it]+dt*K1/2)
            K3 = -1j*np.dot(self.Htot,self.Jmol0Cj[:,it]+dt*K2/2)
            K4 = -1j*np.dot(self.Htot,self.Jmol0Cj[:,it]+dt*K3)
            self.Jmol0Cj[:,it] += (K1+2*K2+2*K3+K4)*dt/6
    
    def propagateJmol0Cj_RK4_Troisi_ensemble(self,dt,index):
        ### RK4 propagation 
        for it in range(index):
            self.updateHmol_Troisi_ensemble(self.Xj[:,it])
            K1 = -1j*np.dot(self.Hmol,self.Jmol0Cj[:,it])
            K2 = -1j*np.dot(self.Hmol,self.Jmol0Cj[:,it]+dt*K1/2)
            K3 = -1j*np.dot(self.Hmol,self.Jmol0Cj[:,it]+dt*K2/2)
            K4 = -1j*np.dot(self.Hmol,self.Jmol0Cj[:,it]+dt*K3)
            self.Jmol0Cj[:,it] += (K1+2*K2+2*K3+K4)*dt/6

    def propagateJcav0Cj_RK4(self,dt):
        ### RK4 propagation 
        K1 = -1j*np.dot(self.Htot,self.Jcav0Cj)
        K2 = -1j*np.dot(self.Htot,self.Jcav0Cj+dt*K1/2)
        K3 = -1j*np.dot(self.Htot,self.Jcav0Cj+dt*K2/2)
        K4 = -1j*np.dot(self.Htot,self.Jcav0Cj+dt*K3)
        self.Jcav0Cj += (K1+2*K2+2*K3+K4)*dt/6

    def propagateJcav0Cj_RK4_ensemble(self,dt,index):
        ### RK4 propagation 
        for it in range(index):
            K1 = -1j*np.dot(self.Htot,self.Jcav0Cj[:,it])
            K2 = -1j*np.dot(self.Htot,self.Jcav0Cj[:,it]+dt*K1/2)
            K3 = -1j*np.dot(self.Htot,self.Jcav0Cj[:,it]+dt*K2/2)
            K4 = -1j*np.dot(self.Htot,self.Jcav0Cj[:,it]+dt*K3)
            self.Jcav0Cj[:,it] += (K1+2*K2+2*K3+K4)*dt/6
    
    def propagateJtot0Cj_RK4(self,dt):
        ### RK4 propagation 
        K1 = -1j*np.dot(self.Htot,self.Jtot0Cj)
        K2 = -1j*np.dot(self.Htot,self.Jtot0Cj+dt*K1/2)
        K3 = -1j*np.dot(self.Htot,self.Jtot0Cj+dt*K2/2)
        K4 = -1j*np.dot(self.Htot,self.Jtot0Cj+dt*K3)
        self.Jtot0Cj += (K1+2*K2+2*K3+K4)*dt/6

    def propagateJtot0Cj_RK4_ensemble(self,dt,index):
        ### RK4 propagation 
        for it in range(index):
            K1 = -1j*np.dot(self.Htot,self.Jtot0Cj[:,it])
            K2 = -1j*np.dot(self.Htot,self.Jtot0Cj[:,it]+dt*K1/2)
            K3 = -1j*np.dot(self.Htot,self.Jtot0Cj[:,it]+dt*K2/2)
            K4 = -1j*np.dot(self.Htot,self.Jtot0Cj[:,it]+dt*K3)
            self.Jtot0Cj[:,it] += (K1+2*K2+2*K3+K4)*dt/6
