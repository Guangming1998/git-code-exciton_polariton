...
if 'param.in' in path:
    ...
    if atomic_unit:
        ...
        hbar = 1
        dt = 0.025e-3*ps_to_au
        couplingstrength = 10 *wavenumber_to_au
        cavityFrequency = -2*staticCoup
    else: #"use amu*A^2*ps^-2 unit"
        staticCoup = 300 *wavenumber_to_amuAps
        dynamicCoup = 995 * wavenumber_to_amuAps
        kBT = 104.3*wavenumber_to_amuAps
...
model1.initialGaussian_ensemble(index=35)
...
if useDiagonalDisorder or SanityCheck or useCavityHamiltonian:
    ...
    # model1.initialCj_cavity_ensemble(kBT)
    model1.initialCj_Troisi_ensemble(kBT)
...
# else:
    ...
    # CJJavg1_list = [model1.getCurrentCorrelation_ensemble(index=35)[2]]
    # CJJavg_cav_list =[model1.getCurrentCorrelation_ensemble(index=35)[1]]
    CJJavg_mol_list = [model1.getCurrentCorrelation_Troisi_ensemble(index=35)]
    ...
    for i in range(Ntimes):
        ...
        model1.old_Aj_ensemble(index=35,Ehrenfest=True)
        
        # model1.RK4_Cj_cavity_ensemble(dt,index=35)
        model1.RK4_Cj_Troisi_ensemble(dt,index=35)
        # model1.propagateJcav0Cj_RK4_ensemble(dt,index=35)
        # model1.propagateJtot0Cj_RK4_ensemble(dt,index=35)
        # model1.propagateJmol0Cj_RK4_ensemble(dt,index=35)
        model1.propagateJmol0Cj_RK4_Troisi_ensemble(dt,index=35)
        
        # model1.velocityVerlet(dt,Ehrenfest)
        model1.velocityVerlet_ensemble(dt,index=35)
        # if i%500 ==0:
        ...
        if i%Nskip ==0:
            ...
            # CJJavg1_list.append(model1.getCurrentCorrelation_ensemble(index=35)[2])
            # CJJavg_cav_list.append(model1.getCurrentCorrelation_ensemble(index=35)[1])
            CJJavg_mol_list.append(model1.getCurrentCorrelation_Troisi_ensemble(index=35))
    ...
    # CJJavg_cav_list = np.array(CJJavg_cav_list,complex)
    CJJavg_mol_list = np.array(CJJavg_mol_list,complex)
    dici = {'CJJ_mol':CJJavg_mol_list}
